...
def find_nominees(data, awards):
    ...
    for award in awards:
        ...
        likely_nominees = [chunk.text for seq in tagged_text for chunk in seq.ents if chunk.label_ in allowed_labels ]
        counter = Counter(likely_nominees)
        nominees[award] = [item[0] for item in counter.most_common(5)]

    return nominees

def _preprocess(data):
    text_lst = [post['text'] for post in data]
    search_words = ['nominee', 'nominated', 'nominees', 'nominating']
